...
//				gradesArr[n]['birthday']=rows[k][$.i18n.prop('birthday')]?new Date((rows[k][$.i18n.prop('birthday')] - (25567+2))*86400*1000).Format("yyyy-MM-dd"):"";
				gradesArr[n]['point']=rows[k][$.i18n.prop('point')]
				isFound=1;
				break;
			}
		}
...
//    alert(classno);
//    http://7xtez1.com2.z0.glb.clouddn.com/student/queryStudentsAdv?classno=F1502&rows=3&page=2&sidx=id&sord=desc
...
var isNoRealData=0;
//http://7xtez1.com2.z0.glb.clouddn.com/grade/queryExistedGrades?classno=M1303&exam_id=38&subject=English
function searchClassnoSubjectExam(){
	var classno=$("#class").val();
	var subject=$("#subject").val();
	if(classno && subject && exam_id){
		$.ajax({ 'async': true,
            'url': '/grade/queryExistedGrades',
            'type': 'get',
...
//    alert(JSON.stringify(gradesArr));
    ...
    var url='/grade/addGrades?';
    for(var k=0;k<gradesArr.length;k++){
        var x=gradesArr[k];
        url+="student_ids="+x["student_id"]+"&";
        url+="points="+(x["point"]?x["point"]:"-1")+"&";
    }
//    alert(url);
